defmodule Benchee.Formatters.ConsoleTest do
  ...
  test ".format_job sorts the the given stats fastest to slowest" do
    jobs = %{
      "Second" => %{
        average: 200.0, ips: 5_000.0, std_dev_ratio: 0.1, median: 195.5
      },
    ...
    [_header, result_1, result_2, result_3 | _dont_care ] =
      Console.format_job(jobs, @console_config)
  ...
  test ".format_job adjusts the label width to longest name" do
    jobs = %{
      "Second" => %{
        average: 200.0, ips: 5_000.0, std_dev_ratio: 0.1, median: 195.5
      },
    ...
    [header, result_1, result_2, result_3 | _dont_care ] =
      Console.format_job(longer_jobs, @console_config)
  ...
  test ".format_job creates comparisons" do
    jobs = %{
      "Second" => %{
        average: 200.0, ips: 5_000.0, std_dev_ratio: 0.1, median: 195.5
      },
  ...
  test ".format_job can omit the comparisons" do
    jobs = %{
      "Second" => %{
        average: 200.0, ips: 5_000.0, std_dev_ratio: 0.1, median: 195.5
      },
    ...
    output =  Enum.join Console.format_job(
                jobs,
                %{
                  comparison:   false,
                  unit_scaling: :best
  ...
  test ".format_job adjusts the label width to longest name for comparisons" do
  ...
  test ".format_job doesn't create comparisons with only one benchmark run" do
    jobs  = %{"First" => %{average: 100.0, ips: 10_000.0,
                           std_dev_ratio: 0.1, median: 90.0}}

    assert [header, result] = Console.format_job jobs, @console_config
    refute Regex.match? ~r/(Comparison|x slower)/, Enum.join([header, result])
  end

  test ".format_job formats small averages and medians more precisely" do
    fast = %{"First" => %{average: 0.15, ips: 10_000.0,
                          std_dev_ratio: 0.1, median: 0.0125}}

    assert [_, result] = Console.format_job fast, @console_config
  ...
  test "it doesn't output weird 'e' formats" do
    ...
    assert [_, result] = Console.format_job jobs, @console_config
