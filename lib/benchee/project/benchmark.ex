defmodule Benchee.Project.Benchmark do
  ...
  defmacro benchmark(name, opts \\ [], [do: {:__block__, [], exprs}]) do
    ...
    inputs =
      ...
      for {:input, _, [name, expr]} <- exprs, do:
        {name, quote do unquote(block_to_fun(expr)).() end}
    ...
    run_fun_name =
      ["run",
       :erlang.unique_integer() |> Integer.to_string(),
       name ]
      |> Enum.join("_")
      |> String.to_atom
  ...
  defp block_to_fun(doblock, args) do
    case Enum.map(args, &atom_to_var/1) do
      [] -> quote do fn -> unquote(doblock) end end
        argvars -> quote do fn (unquote_splicing(argvars)) -> unquote(doblock) end end
    end
  end
end
